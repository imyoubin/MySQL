...
*/
select 	round( max(salary),2) '최고임금',
		round(min(salary),2) '최저임금',
        round(max(salary)-min(salary),2) '최고임금-최저임금'		
from employees
;
...
ORDER BY avg(salary) - min(salary), 2 desc
;
/*
문제8.
...
select  hire_date,
		case
			when hire_date<'2002-12-31' then '창립맴버'
            when year(hire_date)='2003' then '03년입사'
            when year(hire_date)='2004' then '04년입사'
            else '상장이후입사'
        end  as optDate
from employees 
;
/*
문제11 - 필요한 함수(특정날짜의 요일을 수자로 알려줌)를 검색하고 사용법을 주석으로 남겨
두세요
가장 오래 근속한 직원의 입사일은 언제인가요? 다음 형식으로 출력해주세요.
예) 2005년 08월 20일(토요일)
*/